...
func Fonte() {
	transito := 0 // numero de msgs em transito sem confirmacao
	contConf := 0
	for {
		if transito < (T) { // msgs em transito menor que tamanho do buffer!!!
			select {
		...
		} else { // so pode aguardar confirmacao
			<-confirma
			transito--
			contConf++
		}
